...
class radio_env:
    def __init__(self):
        ...
        self.F_BB = np.random.uniform(size=(self.SP.Nt_RF, self.SP.Ns, self.SP.Nu, self.SP.Nc)) + 1j*np.random.random(size=(self.SP.Nt_RF, self.SP.Ns, self.SP.Nu, self.SP.Nc))  # a BS <-> a user Nr X Nt
        self.W_BB = np.random.uniform(size=(self.SP.Nr_RF, self.SP.Ns_peruser, self.SP.Nu, self.SP.Nc)) + 1j*np.random.uniform(size=(self.SP.Nr_RF, self.SP.Ns_peruser, self.SP.Nu, self.SP.Nc))
    ...
    def step(self, action):
        ...
        return 10*np.log10(self.SINR), 10*np.log10(self.SINR_sum), done
    ...
    def compute_SINR(self):
        ...
        self.SINR_sum = np.sum(self.SINR)
