    ...
    def step(self, action):
       # assert self.action_space.contains(action), "%r (%s) invalid"%(action, type(action))
        ...
        if (action != -1): # optimal
            ...
            # beamforming always gets a reward.
            reward += 3
        ...
        abort = (pt_serving > self.max_tx_power) or (pt_interferer > self.max_tx_power_interference) or (received_sinr < self.min_sinr) or (received_ue2_sinr < self.min_sinr) \
            or (received_sinr > 50 + self.sinr_target) or (received_ue2_sinr > 50 + self.sinr_target) or (received_sinr < 10) or (received_ue2_sinr < 10)  
